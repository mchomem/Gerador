...
public class CtrConfiguracao implements InternalFrameListener, ActionListener, ListSelectionListener, ItemListener {
	...
	// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : insernção de variávels.
	...
	public CtrConfiguracao(FrmMenu frmMenu) {
		...
		// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : insernção de evento para getCmbSgbdr.
	...
	// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : alimentando JComboBox com chave SGBDR.
	private void popularComboSgbdr() {
		
		// MISAEL - 28/07/2015 - correção de bug, combo sera populada mais de uma vez, utilização de DefaultComboBoxModel.
	...
	// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : carregamento da imagem.
	private void carregarImagemLogoSgbdr(String nome) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this.frmConfiguracao
					, "Falha ao carregar a imagem. Causa: " + e.getMessage()
					, "Erro"
	...
	// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : implementação de evento ao mudarl valor na JComboBox.
	@Override
	...
	public void internalFrameActivated(InternalFrameEvent e) {
		...
		// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : chmada de métodos.
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : implementação do campo ao mudar valor na JTable.
	...
	private void inicializar() {
		...
		// MISAEL 27/07/2015 - removido por que esta função conflita com o componente UCImageLoader.
		// FormUtils.inicializarCampos(this.frmConfiguracao);
		
		// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : campos tiverem que ser incluidos programaticamente.
		// por conta da falha do método FormUtils.inicializarCampos.
		this.frmConfiguracao.getTxtNome()     .setText("");
		// MISAEL 27/07/2015 - A combo getCmbSgbdr não será incializada aqui pois não contem indice sem elemento!
	...
	private void gravarArquivo() {
		...
		// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : inclusão do campo para ser gravado no arquivo properties.
		...
		if(perfilConexao  .equals("")
			...
			JOptionPane.showMessageDialog(this.frmConfiguracao,
					"Informe os campos obrigatórios para gravação.",
		...
		// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : verificação programática pois o método
		...
		try {
			...
			// MISAEL - 27/07/2015 - tarefa Implementação de Imagem do Logo, e chave SGDBR no formuláiro de configuração de perfil de conexão : inclusão com atributo sgbdr.
			VoArquivoConfiguracao voArquivoConfiguracao = new VoArquivoConfiguracao(perfilConexao, sgbdr, usuario, url, driver, senha, descricao, data_hora);
			...
			JOptionPane.showMessageDialog(this.frmConfiguracao
					, "Perfil de conexão \"" + perfilConexao + "\" gravado com sucesso."
					, InfoProjeto.obterNomeEVersao()
					, JOptionPane.INFORMATION_MESSAGE);			
			
		} catch(IOException e) {
			
			JOptionPane.showMessageDialog(this.frmConfiguracao, "Falha ao gravar arquivo.\n\n[" + e.getMessage() + "]", "Mensagem", JOptionPane.ERROR_MESSAGE);
	...
	private void excluirArquivo() {
		...
		if(perfilConexao.equals("")) {
			...
			JOptionPane.showMessageDialog(this.frmConfiguracao
					, "Informe um Perfil de conexão para exclusão."
					, InfoProjeto.obterNomeEVersao()
					, JOptionPane.WARNING_MESSAGE);
			return;
		...
		int resultado = JOptionPane.showConfirmDialog(this.frmConfiguracao
				  , "Deseja excluir o perfil \"" + perfilConexao + "\"?"
				  , InfoProjeto.obterNomeEVersao()
		...
		if(resultado == JOptionPane.YES_OPTION) {
			...
			try {
				...
				if(this.daoArquivoConfiguracao.consultar(perfilConexao) == null) {
					...
					JOptionPane.showMessageDialog(this.frmConfiguracao
							  , "O Perfil escolhido não foi localizado para exclusão."
							  , InfoProjeto.obterNomeEVersao()
							  , JOptionPane.INFORMATION_MESSAGE);
					return;
				...
				JOptionPane.showMessageDialog(this.frmConfiguracao
						  , "Perfil \"" + perfilConexao + "\" excluido com sucesso."
						  , InfoProjeto.obterNomeEVersao()
						  , JOptionPane.INFORMATION_MESSAGE);
