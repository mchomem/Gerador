...
public class DaoMetadado {
	 ...
	 * @throws IOException
	 */
	public VoMetadado obterMetaDados(Connection conexao)
	...
	private List<VoTabela> obterMTTabela(Connection conexao) throws SQLException {
		...
		while (rsTabela.next()) {
			...
			// Correção: detectado que objetos "views" de banco estavam sendo inclusas na
			// geração de código fonte.
			// 27/01/2015 - Detectado que o resultSet estava obtendo além das tabelas as
			// views do banco.
			// Solução: implementar filtro para descartar objetos de banco que não sejam
			// tabelas.

			// TODO implementação futura, para gerar relatórios um objeto de banco view pode
			// ser útil.

			// Se não for uma tabela (se for uma view por exemplo ou tabela global ou do
			// sistema)
			String tipoObjetoBanco = rsTabela.getString("TABLE_TYPE");

			// TODO esse teste é apenas para testar o postgre, remover depois caso haja a
			// separação (MySql e PostgreSQL) na obtenção de datatype.
			if (tipoObjetoBanco == null) {
				// teste rápido por conta que o postgre não identifica esse objeto null:
				// pg_toast_11708.
				return tabelas;
			}
	...
	/**
	 * Consulta uma coluna para averificar se esta coluna é chave primária na tabela
	 * ou não.
	 * 
	 ...
	 * @throws SQLException
	 */
	private boolean confirmaChavePrimaria(Connection conexao, String nomeTabela, String nomeColuna)
			throws SQLException {
	...
	private List<VoColuna> obterMTColuna(Connection conexao, String nomeTabela) throws SQLException {
		...
		while (rsColuna.next()) {
			...
			System.out.println(rsColuna.getString("COLUMN_NAME") + " - DATA_TYPE(código: " + dataType + " - "
					+ rsColuna.getString("TYPE_NAME") + ")");
			...
			// 06/10/2015
			// TODO chamar o método getImportedKeys aqui, e alimentar uma estrutura de dados
			// (Classe ForeignKey)
			...
			// voColuna.setNotNull((rsColuna.getString("IS_NULLABLE").equals("NO") ? true :
			// false));
	...
	// MISAEL - 07/10/2015 - imlpementação do método para buscar os metadados para
	// se montar a associação na ORM.
	...
	// MISAEL - 03/02/2015 - revisão dos tipos de dados implementados conforme o
	// link e a tabela do livro
	// ver
	...
	private EnTipoDado fazerDeParaJavaMySql(String tipoDadoBanco) {
		...
		case "TINYINT UNSIGNED": // não tem definição no livro e nem no site, ficou definido como "int" conforme
									// consta no banco MySql.
		case "INT": // OK
		case "INTEGER": // OK
		case "MEDIUMINT": // OK
		...
		case "YEAR": // no caso do YEAR do Mysql, conforme configuração do banco, pode ser um short
						// ou um date(com o dia e mês configurado para 01).
		...
		default:
			// TODO verificar, aqui muito certo pode ser usado o Object (pois no javax.sql o
			// Object pode ser utilizado para tudo!)
	...
	private EnTipoDado fazerDeParaJavaPostrgre(String tipoDadoBanco) {
		...
		default:
			// TODO verificar, aqui muito certo pode ser usado o Object (pois no java.sql o
			// Object pode ser utilizado para tudo!)
			enTipoDado = EnTipoDado.DESCONHECIDO;
			break;
