...
public class BoResultadoVoCSNet {
	...
	public String formatarClasseEntidadeCSNet(VoTabela voTabela) {
		...
		for(VoColuna voColuna : voTabela.getColunas()) {
			...
			codigoFonte.append(this.identarComEspaco(2) + "private " + this.deParaTiposBancoParaCSNet(voColuna) + " " + nomeAtributo + ";\r\n");
				...
				if(voTabela.getVoForeignKey() != null) {
					...
					for(VoForeignKey voForeignKey : voTabela.getVoForeignKey()) {
						...
						if(voForeignKey.getNomeColunaFK().equals(voColuna.getNomeColuna())) {
							...
							substitui = true;
							codigoFonte.append(this.identarComEspaco(2)	+ "/// <param name=\"" + this.ajustarNomeAtributo(voForeignKey.getNomeTabelaPai()) + "\"></param>\r\n");
		...
		for(VoColuna voColuna : voTabela.getColunas()) {
			...
			if(voTabela.getVoForeignKey() != null) {
				...
				for(VoForeignKey voForeignKey : voTabela.getVoForeignKey()) {
					...
					if(voForeignKey.getNomeColunaFK().equals(voColuna.getNomeColuna())) {
						...
						// get e set
						codigoFonte.append(this.identarComEspaco(2) + "public " + nomeMetodoFK + " " + nomeMetodoFK + "\r\n");
						codigoFonte.append(this.identarComEspaco(2) + "{\r\n");
			...
			codigoFonte.append(this.identarComEspaco(2) + "public " + this.deParaTiposBancoParaCSNet(voColuna) + " " + nomeMetodo + "\r\n");
			codigoFonte.append(this.identarComEspaco(2) + "{\r\n");
	...
	public String formatarClasseNegocioCSNet(VoTabela voTabela) {
		...
		codigoFonte.append(this.identarComEspaco(2) + "private " + entidade + " " + entidade.toLowerCase() + ";\r\n\r\n");
	
		// Construtor da classe
		codigoFonte.append(this.identarComEspaco(2) + "public " + nomeClasse + "() {}\r\n\r\n");
	
		// 1º Sobrecarga do construtor.
		codigoFonte.append(this.identarComEspaco(2) + "public " + nomeClasse + "(" + entidade + " " + entidade.toLowerCase() + ")\r\n" );
		codigoFonte.append(this.identarComEspaco(2) + "{\r\n");
		codigoFonte.append(this.identarComEspaco(3) + "this." + entidade.toLowerCase() + " = " + entidade.toLowerCase() + ";\r\n");
		codigoFonte.append(this.identarComEspaco(2) + "}\r\n\r\n");
	...
	public String formatarClasseDaoCSNet(VoTabela voTabela) {
		...
		codigoFonte.append(this.identarComEspaco(2) + "}\r\n\r\n");
			...
			codigoFonte.append(this.identarComEspaco(4) + "sql.Append(\"INSERT INTO " + voTabela.getNomeTabela().toUpperCase() + "\");\r\n");
			...
			for(VoColuna voColuna : voTabela.getColunas()) {
				...
				// Se é a última coluna não insere a vírgula.
				if(i == voTabela.getColunas().size()) {
					codigoFonte.append(voColuna.getNomeColuna().toUpperCase() + " = @" + voColuna.getNomeColuna().toUpperCase());
				} else {
					codigoFonte.append(voColuna.getNomeColuna().toUpperCase() + " = @" + voColuna.getNomeColuna().toUpperCase() + ", ");
				}
			}
			...
			codigoFonte.append(this.identarComEspaco(4) + "sql.Append(\"DELETE FROM " + voTabela.getNomeTabela().toUpperCase()  + "\");\r\n");
			...
			codigoFonte.append(this.identarComEspaco(4) + "sql.Append(\" " + voTabela.getNomeTabela().toUpperCase() + "\");\r\n\r\n");
			...
			for(VoColuna voColuna : voTabela.getColunas()) {
				...
				if(i == 1) {
					codigoFonte.append(this.identarComEspaco(5)
							+ entidade + " " + this.ajustarNomeAtributo(voTabela.getNomeTabela())
							+ " = new " + entidade + "();\r\n\r\n");
				}
				...
				if(i == voTabela.getColunas().size()) {
					...
					codigoFonte.append(this.identarComEspaco(5) + this.ajustarNomeAtributo(voTabela.getNomeTabela()) + "."
							+ this.ajustarNomeClassePropriedadeCSNet(voColuna.getNomeColuna()) + " = "
							+ this.inserirConvertCSNet(voColuna.getTipoDado(),  "reader[\"" + voColuna.getNomeColuna().toUpperCase() + "\"].ToString()"));
					codigoFonte.append(";\r\n\r\n");
			...
			codigoFonte.append(this.identarComEspaco(4) + "return " + this.ajustarNomeAtributo(voTabela.getNomeTabela())  + "s;\r\n\r\n");
	...
	public String formatarClasseDaoConexaoCSNet() {
		...
		// MySql ou PostgreSQL ?
		if(SessaoGerador.enSGBDREscolhido == EnSGBDR.MYSQL) {
			codigoFonte.append(this.identarComEspaco(2) + "private String stringConnection = @\"Server=localhost;Port=3306;Database=<BASE DE DADOS>;Uid=<USUARIO>;Pwd=<SENHA>;\";\r\n\r\n");
		} else if(SessaoGerador.enSGBDREscolhido == EnSGBDR.POSTGRESQL) {
			codigoFonte.append(this.identarComEspaco(2) + "private String stringConnection = @\"Server=localhost;Port=5432;Database=<BASE DE DADOS>;Uid=<USUARIO>;Pwd=<SENHA>;\";\r\n\r\n");
		}
		// Construtor.
		...
		codigoFonte.append(this.identarComEspaco(4) + providerAdoNet + " conn = new " + providerAdoNet + "(stringConnection);\r\n");
		...
		// MySql ou PostgreSQL ?
		if(SessaoGerador.enSGBDREscolhido == EnSGBDR.MYSQL) {
			codigoFonte.append(this.identarComEspaco(2) + "public void Desconectar(MySqlConnection conn, MySqlCommand command, MySqlDataReader reader)\r\n");
		} else if(SessaoGerador.enSGBDREscolhido == EnSGBDR.POSTGRESQL) {
			codigoFonte.append(this.identarComEspaco(2) + "public void Desconectar(NpgsqlConnection conn, NpgsqlCommand command, NpgsqlDataReader reader)\r\n");
		}
...
/*		boolean impBigInteger = false;
		...
		/*	for(VoColuna voColuna : voTabela.getColunas()) {
			...
			if(voColuna.getTipoDado() ==  EnTipoDado.JAVA_BIGINTEGER && impBigInteger == false) {
				_using.append("import java.math.BigInteger;\r\n");
				impBigInteger = true;
			}
			...
			if(voColuna.getTipoDado() ==  EnTipoDado.JAVA_TIMESTAMP && impTimestamp == false) {
				_using.append("import java.sql.Timestamp;\r\n");
				impTimestamp = true;			
			}
			
			if(voColuna.getTipoDado() ==  EnTipoDado.JAVA_BIGDECIMAL && impBigDecimal == false) {
				_using.append("import java.math.BigDecimal;\r\n");
				impBigDecimal = true;
			}
