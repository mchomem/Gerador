...
public class BoResultadoVoJava {
	...
	// leitura dos fontes no Notepad do Windows.
	...
	public String formatarClasseEntidadeJava(VoTabela voTabela) {
		...
		for (VoColuna voColuna : voTabela.getColunas()) {
			...
			if (voTabela.getVoForeignKey() != null) {
				...
				for (VoForeignKey voForeignKey : voTabela.getVoForeignKey()) {
					...
					if (voForeignKey.getNomeColunaFK().equals(voColuna.getNomeColuna())) {
						...
						substitui = true;
						codigoFonte.append(this.identarComEspaco(1) + "private "
			...
			codigoFonte.append(this.identarComEspaco(1) + "private " + this.deParaTiposBancoParaJava(voColuna) + " "
					+ nomeAtributo + ";\r\n");
				...
				if (voTabela.getVoForeignKey() != null) {
					...
					for (VoForeignKey voForeignKey : voTabela.getVoForeignKey()) {
						...
						if (voForeignKey.getNomeColunaFK().equals(voColuna.getNomeColuna())) {
							...
							substitui = true;
							codigoFonte.append(this.identarComEspaco(1) + " * @param "
									+ this.ajustarNomeAtributo(voForeignKey.getNomeTabelaPai()) + "\r\n");
			...
			if (voTabela.getVoForeignKey() != null) {
				...
				for (VoForeignKey voForeignKey : voTabela.getVoForeignKey()) {
					...
					if (voForeignKey.getNomeColunaFK().equals(voColuna.getNomeColuna())) {
						...
						substitui = true;
						codigoFonte.append(this.identarComEspaco(2) + "this."
								+ this.ajustarNomeAtributo(voForeignKey.getNomeTabelaPai()) + " = "
								+ this.ajustarNomeAtributo(voForeignKey.getNomeTabelaPai()) + ";\r\n");
		...
		if (gerarComentario) {
			...
			codigoFonte.append(
					this.identarComEspaco(1) + "/* Encapsulamento dos campos da classe (Getters e Setters). */\r\n");
		}
		...
		for (VoColuna voColuna : voTabela.getColunas()) {
			...
			if (voTabela.getVoForeignKey() != null) {
				...
				for (VoForeignKey voForeignKey : voTabela.getVoForeignKey()) {
					...
					if (voForeignKey.getNomeColunaFK().equals(voColuna.getNomeColuna())) {
						...
						// set
						codigoFonte.append(this.identarComEspaco(1) + "public void set" + nomeMetodoFK + "("
								+ nomeMetodoFK + " " + nomeAtributoParametroFK + ") {\r\n");
						codigoFonte.append(this.identarComEspaco(2) + "this." + nomeMetodoFK.toLowerCase() + " = "
								+ nomeAtributoParametroFK + ";\r\n");
						codigoFonte.append(this.identarComEspaco(1) + "}\r\n\r\n");
			...
			codigoFonte.append(this.identarComEspaco(1) + "public " + this.deParaTiposBancoParaJava(voColuna) + " "
					+ prefixo + nomeMetodo + "() {\r\n");
			codigoFonte.append(this.identarComEspaco(2) + "return " + nomeAtributo + ";\r\n");
			codigoFonte.append(this.identarComEspaco(1) + "}\r\n\r\n");

			// set
			codigoFonte.append(this.identarComEspaco(1) + "public void set" + nomeMetodo + "("
					+ this.deParaTiposBancoParaJava(voColuna) + " " + nomeVariavelParametro + ") {\r\n");
			codigoFonte.append(
					this.identarComEspaco(2) + "this." + nomeAtributo + " = " + nomeVariavelParametro + ";\r\n");
			codigoFonte.append(this.identarComEspaco(1) + "}\r\n\r\n");
	...
	// MISAEL - 17/08/2015 - método para formatar o código fonte para as classes de
	// negócio.
	public String formatarClasseNegocioJava(VoTabela voTabela) {
		...
		codigoFonte
				.append(this.identarComEspaco(1) + "private " + entidade + " " + entidade.toLowerCase() + ";\r\n\r\n");

		// Construtor da classe
		codigoFonte.append(this.identarComEspaco(1) + "public " + nomeClasse + "() {}\r\n\r\n");

		// 1º Sobrecarga do construtor.
		codigoFonte.append(this.identarComEspaco(1) + "public " + nomeClasse + "(" + entidade + " "
				+ entidade.toLowerCase() + ") {\r\n");
		codigoFonte.append(
				this.identarComEspaco(2) + "this." + entidade.toLowerCase() + " = " + entidade.toLowerCase() + ";\r\n");
		codigoFonte.append(this.identarComEspaco(1) + "}\r\n\r\n");
		...
		// set.
		codigoFonte.append(this.identarComEspaco(1) + "public void set" + entidade + "(" + entidade + " "
				+ entidade.toLowerCase() + ") {\r\n");
		codigoFonte.append(
				this.identarComEspaco(2) + "this." + entidade.toLowerCase() + " = " + entidade.toLowerCase() + ";\r\n");
		codigoFonte.append(this.identarComEspaco(1) + "}\r\n\r\n");
	...
	// MISAEL - 08/10/2015 - método para formatar o código fonte para as classes
	// DAO.
	public String formatarClasseDaoJava(VoTabela voTabela) {
		...
		codigoFonte.append(this.identarComEspaco(1) + "public void inserir" + "(" + entidade + " "
		...
		codigoFonte.append(this.identarComEspaco(2) + "sql.append(\"INSERT INTO "
		...
		for (VoColuna voColuna : voTabela.getColunas()) {
			...
			codigoFonte.append(this.identarComEspaco(2) + "pstm.setObject(" + i + ", "
					+ this.ajustarNomeAtributo(voTabela.getNomeTabela()) + "." + prefixo
					+ this.ajustarNomeClassePropriedadeJava(voColuna.getNomeColuna()) + "());\r\n");
		...
		codigoFonte.append(this.identarComEspaco(1) + "public void alterar" + "(" + entidade + " "
				+ this.ajustarNomeAtributo(voTabela.getNomeTabela()) + ")"
				+ " throws ClassNotFoundException, SQLException {\r\n\r\n");
		...
		// Não ZERAR a variável "i" aqui neste ponto, manter o seu valor para manter a
		// sequência dos parâmentros do PreparedStatement
		for (String colPK : colunasPKAlterar) {
			i++;
			codigoFonte.append(this.identarComEspaco(2) + "pstm.setObject(" + i + ", "
					+ this.ajustarNomeAtributo(voTabela.getNomeTabela()) + ".get"
					+ this.ajustarNomeClassePropriedadeJava(colPK) + "());\r\n");
		...
		codigoFonte.append(this.identarComEspaco(1) + "public void excluir" + "(" + entidade + " "
				+ this.ajustarNomeAtributo(voTabela.getNomeTabela()) + ")"
				+ " throws ClassNotFoundException, SQLException {\r\n\r\n");
		...
		codigoFonte.append(this.identarComEspaco(2) + "sql.append(\"DELETE FROM "
				+ voTabela.getNomeTabela().toUpperCase() + "\");\r\n");
		...
		codigoFonte.append(this.identarComEspaco(1) + "public List<" + entidade + "> consultar()"
				+ " throws ClassNotFoundException, SQLException {\r\n\r\n");

		codigoFonte.append(this.identarComEspaco(2) + "Connection conn        = this.dao.conectar();\r\n");
		codigoFonte.append(this.identarComEspaco(2) + "PreparedStatement pstm = null;\r\n");
		codigoFonte.append(this.identarComEspaco(2) + "List<" + entidade + "> "
				+ this.ajustarNomeAtributo(voTabela.getNomeTabela()) + "s" + " = new ArrayList<" + entidade
				+ ">();\r\n");
		...
		codigoFonte.append(
				this.identarComEspaco(2) + "sql.append(\" " + voTabela.getNomeTabela().toUpperCase() + "\");\r\n\r\n");
		...
		for (VoColuna voColuna : voTabela.getColunas()) {
			...
			if (i == 1) {
				codigoFonte.append(this.identarComEspaco(3) + entidade + " "
			...
			if (i == voTabela.getColunas().size()) {
				codigoFonte.append(this.identarComEspaco(3) + this.ajustarNomeAtributo(voTabela.getNomeTabela())
						+ ".set" + this.ajustarNomeClassePropriedadeJava(voColuna.getNomeColuna()) + "(rs.get"
						+ this.ajustarNomeClassePropriedadeJava(this.deParaTiposBancoParaJava(voColuna)) + "(\""
		...
		codigoFonte.append(this.identarComEspaco(2) + "return " + this.ajustarNomeAtributo(voTabela.getNomeTabela())
				+ "s;\r\n\r\n");
	...
	public String formatarClasseDaoConexaoJava() {
		...
		if (SessaoGerador.enSGBDREscolhido == EnSGBDR.MYSQL) {
			codigoFonte.append(this.identarComEspaco(1)
					+ "private static final String URL          = \"jdbc:mysql://localhost:3306/<BASE DE DADOS>\";\r\n");
			codigoFonte.append(this.identarComEspaco(1)
					+ "private static final String DRIVER_CLASS = \"com.mysql.jdbc.Driver\";\r\n");
			codigoFonte
					.append(this.identarComEspaco(1) + "private static final String USER         = \"<USUÁRIO>\";\r\n");
			codigoFonte.append(
					this.identarComEspaco(1) + "private static final String PASS         = \"<SENHA>\";\r\n\r\n");
		} else if (SessaoGerador.enSGBDREscolhido == EnSGBDR.POSTGRESQL) {
			codigoFonte.append(this.identarComEspaco(1)
					+ "private static final String URL          = \"jdbc:postgresql://localhost:5432/<BASE DE DADOS>\";\r\n");
			codigoFonte.append(this.identarComEspaco(1)
		...
		codigoFonte.append(
				this.identarComEspaco(2) + "Connection conn = DriverManager.getConnection(URL, USER, PASS);\r\n");
	...
	// MISAEL - 28/07/2015 - REFATORAÇÃO, AJUSTE DE NOME DE
	// ajustarNomeClassePropriedade PARA ajustarNomeClassePropriedadeJava.
	...
	// MISAEL - 28/07/2015 - REFATORAÇÃO, NOME DO MÉTODO DE inserirImports PARA
	// inserirImportsJava.
	private String inserirImportsJava(VoTabela voTabela) {
		...
		// Caso nenhum import seja utilizado, não aplica espaçamento entre o import e a
		// classe
	...
	// MISAEL - 28/07/2015 - REFATORAÇÃO, RENOMEADO NOME DO MÉTODO DE
	// trazudirMySqlParaJava para deParaTiposMySqlParaJava.
	private String deParaTiposBancoParaJava(VoColuna voColuna) {
		...
		// MISAEL - 26/02/2015: correção, faltou incluir os datatypes Time, double e
		// float.

		String dataType = "";

		// TODO detalhe, no caso da VoColuna no getForeignKey() trouxer o nome da tabela
		// ao qual se referencia, então imprime como
