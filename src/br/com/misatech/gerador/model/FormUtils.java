...
public class FormUtils {
	...
	public static void setCentralizarForm(JFrame pForm) {
		...
		pForm.setLocation((dimensaoMonitor.width - pForm.getWidth()) / 2, (dimensaoMonitor.height - pForm.getHeight()) / 2);
	...
	public static void setCentralizarForm(JInternalFrame pInterForm) {
		...
		pInterForm.setLocation((dimensaoMonitor.width - pInterForm.getWidth()) / 2, (dimensaoMonitor.height - pInterForm.getHeight()) / 2);
	...
	public static boolean verificarCaracterIndevido(Container recipiente) {
		...
		/* Guardar em array de componentes, todos os componentes.
		 * do componente recipiente. */
	...
	/**
	 * Verifica em uma sequência de string a existência de caracteres
	 * que não devem ser usados.
	 * 
	 * @return Retorna true se encontrado um caractere indevido, que
    ...
    public static String formataMensagemExcecao (Throwable thr) {
         ...
         *    int x = 100 / 0;
         * } catch (Exception ex) {
         *    JOptionPane.showMessageDialog (null, formataMensagemExcecao (ex), "Erro", JOptionPane.ERROR_MESSAGE);
         * }
    	 */
